...
from clip_benchmark.utils.utils import _load_features, _check_models
...
def compute_dist_matrix(sim_method, feature_root, model_ids, split, kernel='linear', rsa_method='correlation',
    ...
    if sim_method == 'cka':
        dist_matrix = compute_cka_matrix(model_ids_with_idx, feature_root,
                                         split=split, kernel=kernel, backend=backend,
                                         unbiased=unbiased, device=device, sigma=sigma)
    elif sim_method == 'rsa':
        dist_matrix = compute_rsa_matrix(model_ids_with_idx, feature_root, split=split, rsa_method=rsa_method,
